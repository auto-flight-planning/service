    ...
    <div className="relative flex items-center gap-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
      ...
      {type === "edit" && canRemove && (
        <button
          ...
          className="absolute -top-2 -right-2 w-6 h-6 bg-white hover:bg-red-50 border border-gray-300 hover:border-red-300 text-gray-400 hover:text-red-400 rounded-full flex items-center justify-center transition-all shadow-sm hover:shadow-md group hover:cursor-pointer active:scale-95 active:bg-red-100"
        >
          <svg
            className="w-3 h-3"
            fill="none"
      ...
      <div className="flex-1">
        ...
        />
        {error && (
          <p className="text-red-400 text-xs mt-1 text-right w-full pr-1">
            {error.message}
          </p>
        )}
      </div>
